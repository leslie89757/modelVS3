#!/usr/bin/env python3
"""统一工具注册脚本 - 重新部署后一键注册所有Function Call工具 (Docker版本)"""
...
async def test_tool_api():
    ...
    try:
        ...
        # 根据模式选择API地址（Docker内部访问）
        dev_mode = "--dev" in sys.argv
        api_url = "http://api:8000/api/v1/tools/"  # Docker内部统一使用api:8000
...
async def main():
    ...
    print(f"🔧 ModelVS3 Function Call工具注册脚本 - Docker版本 ({env_name})")
    print(f"⏰ 开始时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"🎯 用途: 为{env_name}注册所有Function Call工具")
    print(f"🐳 Docker模式: 连接postgres服务")
    if dev_mode:
    ...
    # 测试API (跳过API测试，因为在容器内无法访问外部API)
    print("\n🧪 跳过API测试 (容器内环境)")
    
    # 显示完成信息
    print("\n" + "=" * 70)
    if registrar.failed_count == 0:
        print("🎉 所有工具注册成功！")
        print("💡 现在您可以在前端工具管理页面看到所有工具")
        if dev_mode:
            print("🔗 开发环境访问: http://localhost:3004/tools")
        else:
            print("🔗 生产环境访问: http://36.153.25.22:3003/tools")
